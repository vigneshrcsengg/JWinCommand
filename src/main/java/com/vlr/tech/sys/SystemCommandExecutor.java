...
public class SystemCommandExecutor {
    ...
    private Process runCommandHelper(String commandLine) throws IOException {
        ...
        String cmd = "cmd.exe /c " + commandLine;

        if (fWorkingDirectory == null) {
            process = Runtime.getRuntime().exec(cmd, getEnvTokens());
        } else {
            process = Runtime.getRuntime().exec(cmd, getEnvTokens(), new File(fWorkingDirectory));
        }
